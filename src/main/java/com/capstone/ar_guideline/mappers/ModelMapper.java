  ...
  public static Model fromModelCreationRequestToEntity(ModelCreationRequest request) {
    return Model.builder()
            ...
            .position(request.getPosition() != null ?
                    request.getPosition().stream().map(String::valueOf).collect(Collectors.joining(", ")) : null)
            .rotation(request.getRotation() != null ?
                    request.getRotation().stream().map(String::valueOf).collect(Collectors.joining(", ")) : null)
            .build();
  }
