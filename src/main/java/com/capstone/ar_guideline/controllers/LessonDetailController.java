...
public class LessonDetailController {
  ...
  @Autowired ILessonDetailService lessonDetailService;

  @GetMapping(value = ConstAPI.LessonDetailAPI.LESSON_DETAIL + "/lesson/{lessonId}")
  public ApiResponse<List<LessonDetailResponse>> getAllByLessonId(@PathVariable String lessonId) {
    log.info("get all by lesson id: {}", lessonId);
    return ApiResponse.<List<LessonDetailResponse>>builder()
        .result(lessonDetailService.findAllByLessonId(lessonId))
        .build();
  }
  ...
  public ApiResponse<String> deleteLessonDetail(@PathVariable String lessonDetailId) {
