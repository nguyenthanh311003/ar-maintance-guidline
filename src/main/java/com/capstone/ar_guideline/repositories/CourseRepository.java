...
public interface CourseRepository extends JpaRepository<Course, String> {
    ...
    @Query(
            value = "SELECT c FROM Course c WHERE c.company.id = :companyId "
                    + "AND (:title = '' OR :title IS NULL OR LOWER(c.title) LIKE LOWER(CONCAT('%', :title, '%'))) "
                    + "AND (:status = '' OR :status IS NULL OR c.status = :status) "
                    + "ORDER BY c.createdDate DESC"
    )
    Page<Course> findByCompanyId(Pageable pageable, @Param("companyId") String companyId, @Param("title") String title, @Param("status") String status);
