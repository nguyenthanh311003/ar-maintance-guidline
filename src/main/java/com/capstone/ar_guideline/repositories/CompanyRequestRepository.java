...
public interface CompanyRequestRepository extends JpaRepository<CompanyRequest, Long> {
  ...
  @Query(value = "SELECT c FROM CompanyRequest c WHERE c.company.id = :companyId "
      + "AND (:status IS NULL OR LOWER(c.status) LIKE LOWER(CONCAT('%', :status, '%'))) "
      + "ORDER BY c.createdAt DESC")
  Page<CompanyRequest> findByCompanyId(Pageable pageable, @Param("companyId") String companyId, @Param("status") String status);
