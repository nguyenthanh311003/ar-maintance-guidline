...
public interface CompanySubscriptionRepository extends JpaRepository<CompanySubscription, String> {
  ...
  @Query(
      "SELECT cs FROM CompanySubscription cs WHERE cs.subscriptionExpireDate <= :expireDate AND cs.status ='ACTIVE' ")
  List<CompanySubscription> findBySubscriptionExpireDateThatOnlyHave3DaysLeft(
      LocalDateTime expireDate);

  @Query(
      "SELECT cs FROM CompanySubscription cs WHERE cs.subscriptionExpireDate >= CURRENT_TIMESTAMP AND cs.status = 'ACTIVE'")
