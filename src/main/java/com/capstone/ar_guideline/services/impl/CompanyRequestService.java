...
public class CompanyRequestService implements ICompanyRequestService {
    ...
    @Override
    public PagingModel<CompanyRequestResponse> findByCompanyId(int page, int size, String companyId, String status) {
        try {
    ...
    public CompanyRequestResponse update(String requestId, CompanyRequestCreation request) {
        try {
            ...
            if ((request.getStatus().equalsIgnoreCase(COMPANY_CANCELLED) || request.getStatus().equalsIgnoreCase(DRAFTED)) && modelNeedToDelete != null) {
