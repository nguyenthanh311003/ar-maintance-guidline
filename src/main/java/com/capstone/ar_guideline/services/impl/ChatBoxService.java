...
public class ChatBoxService {
    ...
    public ChatBox createChatBox(List<UUID> participants, UUID companyRequestId) {
        ...
        Optional<CompanyRequest> companyRequest = Optional.ofNullable(companyRequestRepository.findByRequestId(String.valueOf(companyRequestId)));
    ...
    public ChatMessage addMessageToChatBox(ChatMessageRequest request) {
        CompanyRequest companyRequest = companyRequestRepository.findByRequestId(String.valueOf(request.getChatBoxId()));
        ChatBox chatBox = chatBoxRepository.findById(companyRequest.getChatBoxes().get(0).getId())
    ...
    public List<ChatMessageResponse> getChatBoxMessages(UUID chatBoxId) {
        CompanyRequest companyRequest =  companyRequestRepository.findByRequestId(String.valueOf(chatBoxId));
        List<ChatMessage> messages = chatMessageRepository.findByChatBoxIdOrderByTimestampAsc(companyRequest.getChatBoxes().get(0).getId());
